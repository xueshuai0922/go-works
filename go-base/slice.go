...
/*
  - go中的slice就是Java中的list 可变数组=动态数组
    实例化过程 ： make([]type,length,capacity ) type指的是数据类型，length是动态数组的长度（实际占用），capacity是数组的容量（可以占用长度）
    make指的是开创空间
*/
func main() {
	//1.切片的第一种方式
	...
	printSlice(numbers)       //len=7 ,cap =7, slice=[1 2 3 4 5 6 7]
	fmt.Println(numbers[1:4]) //[2 3 4]

	fmt.Println(numbers[:4]) //[1 2 3 4]
	fmt.Println(numbers[3:]) //[4 5 6 7]
	numOne := numbers[:3]
	...
	printSlice(numbers)              //len=8 ,cap =14, slice=[1 2 3 4 5 6 7 8] 这里我们可以注意到如果append增加的内容超过了容量，切片容量扩容到2倍
